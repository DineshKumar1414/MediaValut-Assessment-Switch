...
export function AssetDetail({ id, onClose, onSaved }: Props) {
    ...
    try {
      ...
      if (err instanceof ApiError && err.code === 'version_conflict') {
        try {
          ...
          setAsset(latest);
          setError('This asset was changed elsewhere. The latest version is loaded; review it, then try again.');
        } catch (refreshError) {
          setError(userMessage(refreshError));
        }
      } else {
  ...
  return (
    <aside ref={panelRef} className="panel" role="complementary" aria-label="Asset detail" tabIndex={-1} onKeyDown={(event) => { if (event.key === 'Escape') { event.preventDefault(); onClose(); } }}>
