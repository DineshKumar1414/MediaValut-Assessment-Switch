...
import type { Asset, AssetKind, AssetStatus, AssetQuery, BulkResult } from '@/lib/types';
...
const chunks = <T,>(v: T[], size: number) => Array.from({ length: Math.ceil(v.length / size) }, (_, i) => v.slice(i * size, i * size + size));
async function pooled<T>(jobs: (() => Promise<T>)[], limit = 3) { const results: T[] = []; let index = 0; await Promise.all(Array.from({ length: Math.min(limit, jobs.length) }, async () => { while (index < jobs.length) { const job = jobs[index++]!; results.push(await job()); } })); return results; }
...
function MediaVault() {
  ...
  async function applyBulkStatus(nextStatus: AssetStatus, ids = [...selectedIds]) {
    if (!ids.length || !online) return; const snapshot = new Map(assets.filter((a) => ids.includes(a.id)).map((a) => [a.id, a])); setNotice(null); setFailures([]); setFailedStatus(null); patchCache(ids, (a) => ({ ...a, status: nextStatus }));
    try { const results = await pooled(chunks(ids, 50).map((chunk) => () => bulkSetStatus(chunk, nextStatus))); const failed = results.flatMap((r: BulkResult) => r.results.filter((x) => !x.ok)); const applied = results.reduce((n, r) => n + r.applied, 0); if (failed.length) patchCache(failed.map((f) => f.id), (a) => snapshot.get(a.id) ?? a); const details = failed.map((f) => ({ id: f.id, name: snapshot.get(f.id)?.name ?? f.id, code: f.code, message: f.message ?? (f.code === 'legal_hold' ? 'On legal hold; this status cannot be changed.' : 'Changed elsewhere. You can retry.') })); setFailures(details); setFailedStatus(failed.length ? nextStatus : null); setSelectedIds(new Set(details.filter((f) => f.code === 'conflict').map((f) => f.id))); setNotice(failed.length ? `${applied} updated; ${failed.length} need attention.` : `${applied} assets updated.`); }
